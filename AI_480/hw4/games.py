...
alphaBetaCount = 0;
def alphabeta_count_states(state, game):
    player = game.to_move(state)
    ...
    for a in game.actions(state):
        ...
        if v > best_score:
            best_score = v
            best_action = a
    return (best_action, alphaBetaCount+1)
...
def alphabeta_player_count_states(game, state):
    return alphabeta_count_states(state, game)
...
class Game:
    ...
    def who_wins(self, state, player):
...
class TicTacToe(Game):
    ...
    def buildState(self, to_move='X', x_moves=[], y_moves=[], h=3, v=3, k=3):
        moves = set([(x, y) for x in range(1, h + 1) for y in range(1, v + 1)]) - set(x_moves) - set(y_moves)
        moves = list(moves)
        moves.sort()
        board = {}
        for pos in y_moves:
    ...
    def compute_initial_utility(self, list1, list2, list3):
        tempMoves = [list1, list2, list3]
        for row in tempMoves:
            if len(set(row)) == 1 and row[0] != '-':
                player = row[0];
                return +1 if player == 'X' else -1
        for newMoves in [tempMoves, np.transpose(tempMoves)]:
            for row in newMoves:
                if len(set(row)) == 1 and row[0] != '-':
                    player = row[0];
                    return +1 if player == 'X' else -1
        if len(set([tempMoves[i][i] for i in range(len(tempMoves))])) == 1 and tempMoves[0][0] != "-":
            player = tempMoves[0][0]
            return +1 if player == 'X' else -1

        if len(set([tempMoves[i][len(tempMoves)-i-1] for i in range(len(tempMoves))])) == 1 and tempMoves[0][len(tempMoves)-1] != "-":
            player = tempMoves[0][len(tempMoves)-1]
            return +1 if player == 'X' else -1
    ...
    def play_game(self, *players):
        ...
        state = self.initial
        self.display(state)
        if(state.utility == 1 or state.utility == -1 or (state.utility == 0 and len(state.moves) == 0)):
            return
